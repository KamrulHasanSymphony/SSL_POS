var BankInformationController = function (CommonService, CommonAjaxService) {
    ...
    var init = function () {
        ...
        $('#btnPrevious').click('click', function () {
            var getId = $('#Id').val();
            if (parseInt(getId) > 0) {
                window.location.href = "/DMS/BankInformation/NextPrevious?id=" + getId + "&status=Previous";
            }
        });
        $('#btnNext').click('click', function () {
            var getId = $('#Id').val();
            if (parseInt(getId) > 0) {
                window.location.href = "/DMS/BankInformation/NextPrevious?id=" + getId + "&status=Next";
            }
        });
    ...
    function SelectData() {
        ...
        var url = "/DMS/BankInformation/Delete";
    ...
    var GetGridDataList = function () {
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: "/DMS/BankInformation/GetGridData",
                    type: "POST",
                    dataType: "json",
                    cache: false
                },
        ...
        $("#GridDataList").kendoGrid({
            ...
            search: {
                fields: ["Code", "Name", "BanglaName", "Address", "BanglaAddress", "TelephoneNo", "FaxNo", "Email", "Comments"]
            },
            excel: {
            ...
            columns: [
               
                {
                    ...
                    template: function (dataItem) {
                        ...
                        return `
            <a href="/DMS/BankInformation/Edit/${dataItem.Id}" class="btn btn-primary btn-sm mr-2 edit" title="Edit Credit Limit">
    ...
    //                            param.field = "H.Comments";
    //                        }
    //                        if (param.field === "IsArchive") {
    //                            param.field = "H.IsArchive";
    //                        }
...
function DevitCredit(branchId, FromDate, ToDate) {
    ...
    const form = document.createElement('form');
    form.method = 'post';
    form.action = '/DMS/BankInformation/DevitCredit';
    form.target = '_blank';
    const inputVal = document.createElement('input');
    inputVal.type = 'hidden';
    inputVal.name = 'Id';
