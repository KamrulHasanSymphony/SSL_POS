var FiscalYearForSaleController = function (CommonAjaxService) {
    ...
    var init = function () {
        ...
        $("#btnFDt").on('click', function () {
            ...
            let url = '/DMS/FiscalYearForSale/FiscalYearSet?YearStart=' + yearStart + '&YearEnd=' + yearEnd;
    ...
    function SelectData() {
        ...
        var url = "/DMS/FiscalYearForSale/Delete";
    ...
    var GetGridDataList = function () {
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: "/DMS/FiscalYearForSale/GetFiscalYearsGrid",
                    type: "POST",
                    dataType: "json",
                    cache: false
                },
        ...
        $("#FiscalYearsGrid").kendoGrid({
            ...
            search: {
                fields: ["Id", "Year", "YearStart", "YearEnd"]
            },
            columns: [
                {
                ...
                },
                {
                    ...
                    template: function (dataItem) {
                        return `
                            <a href="/DMS/FiscalYearForSale/Edit/${dataItem.Id}" class="btn btn-primary btn-sm mr-2 edit" title="Edit Fiscal Year">
    ...
    $('body').on('click', '.btnDelete-FiscalYear', function (e) {
        ...
        var id = data.split('~')[0];
        var url = "/FiscalYearForSale/DeleteItem?id=" + id + "";
    ...
    function save() {
        ...
        var monthLock = $('#MonthLock').is(':checked');
        fiscalYearDetails.monthLock = monthLock;
        // Iterate through each row in the PartialView table
        var operation = $("#Operation").val();

        if (operation == 'add') {
            $('#fiscalYearDetails .card-body').each(function () {
                var row = $(this);
                //monthLock = row.find('input[name$="#MonthLock"]').is(':checked');
        ...
        fiscal.fiscalYearForSaleDetails = fiscalYearDetails;

        var url = "/DMS/FiscalYearForSale/CreateEdit"
