var SaleReturnDetailsIndexController = function (CommonService, CommonAjaxService) {
    ...
    var GetGridDetailsDataList = function () {
        ...
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: "/SaleReturn/GetDetailsGridData",
                    type: "POST",
                    dataType: "json",
                    cache: false,
        ...
        $("#GridDetailsDataList").kendoGrid({
            ...
            search: ["Code", "BranchName", "CustomerName", "ProductName","UOMName","SalePersonName", "RouteName", "DeliveryAddress", "InvoiceDateTime", "DeliveryDate", "GrandTotalAmount", "GrandTotalSDAmount", "GrandTotalVATAmount", "Comments", "TransactionType", "IsPost", "Quantity", "UnitRate", "SubTotal", "SD", "SDAmount", "VATRate", "VATAmount", "LineTotal", "UOMName", "CurrencyName"],
            excel: {
                fileName: `SaleReturn_Details_${new Date().toISOString().split('T')[0]}.xlsx`,
                filterable: true
            },
            pdf: {
                fileName: `SaleReturn_Details_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0]}.${new Date().getMilliseconds()}.pdf`,
                allPages: true,
                avoidLink: true,
                filterable: true
            },
            pdfExport: function (e) {
                ...
                var fileName = `SaleReturn_Details_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0]}.${new Date().getMilliseconds()}.pdf`;
            ...
            columns: [
                ...
                { field: "VehicleNo", title: "Vehicle No", width: 180, sortable: true },
