...
namespace ShampanPOSUI.Areas.DMS.Controllers
{
    ...
    public class OverHeadController : Controller
    {
        ...
        OverHeadRepo _repo = new OverHeadRepo();
        ...
        [HttpPost]
        public ActionResult CreateEdit(OverHeadVM model)
        {
            ResultModel<OverHeadVM> result = new ResultModel<OverHeadVM>();
            ResultVM resultVM = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new OverHeadRepo();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.Operation.ToLower() == "add")
                    {
                        ...
                        {
                            model = JsonConvert.DeserializeObject<OverHeadVM>(resultVM.DataVM.ToString());
                            model.Operation = "add";
                            //model.ImagePath = model.ImagePath;
                            Session["result"] = resultVM.Status + "~" + resultVM.Message;
                            //model.ImagePath = resultVM.ImagePath;
                            result = new ResultModel<OverHeadVM>()
                            {
                                Success = true,
        ...
        public ActionResult Edit(string id)
        {
            try
            {
                ...
                {
                    vm = JsonConvert.DeserializeObject<List<OverHeadVM>>(result.DataVM.ToString()).FirstOrDefault();
                }
                else
                {
                    vm = null;
        ...
        public JsonResult GetGridData(GridOptions options)
        {
            ...
            try
            {
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var gridData = JsonConvert.DeserializeObject<GridEntity<OverHeadVM>>(result.DataVM.ToString());
