...
function addRow($table) {
    ...
    $table.find('thead th').each(function () {
        ...
        var columName = columnNameAttr || $(this).text();
        columName = columName.replace(/ /g, "-");
        ...
        if (columName == 'SLNo') {
            $cell.append(serialNo);
        }

        // Hide column if th is hidden
        if (isHidden) {
            $cell.attr('hidden', true);
        }

        // Default values for decimal columns
        if (type === 'decimal') {
            var val = (columName === 'ToQuantity') ? 100000 : 0;
            $cell.text(val);
        }
...
//            // Add number input for decimal columns
//            var $input = $('<input type="number" class="form-control txt' + columName + '" value="0" min="0" />');
//            $cell.append($input);
//        } else if (type === 'text') {
//            // Add text input for text columns
//            var $input = $('<input type="text" class="form-control txt' + columName + '" />');
//            $cell.append($input);
