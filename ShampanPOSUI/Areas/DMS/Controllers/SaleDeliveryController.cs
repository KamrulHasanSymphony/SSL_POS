...
namespace ShampanPOSUI.Areas.DMS.Controllers
{
    [Authorize]
    [RouteArea("DMS")]
    public class SaleDeliveryController : Controller
    {
        private readonly ApplicationDbContext _applicationDb;
        SaleDeliveryRepo _repo = new SaleDeliveryRepo();
        CommonRepo _commonRepo = new CommonRepo();

        // GET: DMS/SaleDelivery
        ...
        public ActionResult Create()
        {
            SaleDeliveryVM vm = new SaleDeliveryVM();
            vm.Operation = "add";
			vm.TransactionType = "SaleDelivery";
			var currentBranchId = Session["CurrentBranch"] != null ? Session["CurrentBranch"].ToString() : "0";
            vm.BranchId = Convert.ToInt32(currentBranchId);
        ...
        {
            SaleDeliveryDetail vm = new SaleDeliveryDetail();
        ...
        [HttpPost]
        public ActionResult CreateEdit(SaleDeliveryVM model)
        {
            ResultModel<SaleDeliveryVM> result = new ResultModel<SaleDeliveryVM>();
            ResultVM resultVM = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new SaleDeliveryRepo();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    if (model.Operation.ToLower() == "add")
                    {
                        ...
                        {
                            model = JsonConvert.DeserializeObject<SaleDeliveryVM>(resultVM.DataVM.ToString());
                            model.Operation = "add";
                            Session["result"] = resultVM.Status + "~" + resultVM.Message;
                            result =  new ResultModel<SaleDeliveryVM>()
                            {
                                Success = true,
        ...
        public ActionResult Edit(string id)
        {
            try
            {
                ...
                {
                    vm = JsonConvert.DeserializeObject<List<SaleDeliveryVM>>(result.DataVM.ToString()).FirstOrDefault();
                }
                else
                {
                    vm = null;
        ...
        [HttpPost]
        public ActionResult Delete(SaleDeliveryVM vm)
        {
            ResultModel<SaleDeliveryVM> result = new ResultModel<SaleDeliveryVM>();
        ...
        public JsonResult GetDetailsGridData(GridOptions options, string isPost, string branchId, string fromDate, string toDate)
        {
            ...
            try
            {
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var gridData = JsonConvert.DeserializeObject<GridEntity<SaleDeliveryDetail>>(result.DataVM.ToString());
        ...
        [HttpPost]
        public JsonResult FromSaleGridData(GridOptions options,string branchId, string fromDate, string toDate)
        {
            ResultVM result = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new SaleDeliveryRepo();
        ...
        public ActionResult GetFromSaleOrder(CommonVM vm)
        {
            try
            {
                ...
                SaleDeliveryVM saleDelivery = new SaleDeliveryVM();
                vm.Id = vm.IDs[0];
                ResultVM result = _repoo.SaleOrderList(vm);
                if (result.Status == "Success" && result.DataVM != null)
                {
                    saleDelivery = JsonConvert.DeserializeObject<List<SaleDeliveryVM>>(result.DataVM.ToString()).FirstOrDefault();
                }

                saleDelivery.Operation = "add";
                saleDelivery.IsPost = false;
                saleDelivery.InvoiceDateTime = DateTime.Now.ToString("yyyy-MM-dd");
                saleDelivery.DeliveryDate = DateTime.Now.ToString("yyyy-MM-dd");
                ...
                if (settingsValue.Status == "Success" && settingsValue.DataVM != null)
                {
                    ...
                    saleDelivery.DecimalPlace = string.IsNullOrEmpty(data.SettingValue) ? 2 : Convert.ToInt32(data.SettingValue);
                }

                #endregion

                return View("Create", saleDelivery);
            }
            catch (Exception e)
            {
                Session["result"] = "Fail" + "~" + e.Message;
        ...
        [HttpGet]
        public JsonResult GetSaleDeliveryDetailDataById(GridOptions options, int masterId)
        {
            ResultVM result = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new SaleDeliveryRepo();
            ...
            try
            {
                result = _repo.GetSaleDeliveryDetailDataById(options, masterId);
        ...
        public ActionResult NextPrevious(int id, string status)
        {
            ...
            try
            {
                ...
                vm.TableName = "SaleDeleveries";

                ResultVM result = _commonRepo.NextPrevious(vm);

                if (result.Id != "0")
                {
                    string url = Url.Action("Edit", "SaleDelivery", new { area = "DMS", id = result.Id });
                    return Redirect(url);
                }
                else
                {
                    string url = Url.Action("Edit", "SaleDelivery", new { area = "DMS", id = id });
                    return Redirect(url);
                }
            }
            catch (Exception e)
        ...
        [HttpPost]
        public async Task<ActionResult> IncoiceReportPreview(CommonVM param)
        {
            try
            {
                _repo = new SaleDeliveryRepo();
                param.CompanyId = Session["CompanyId"] != null ? Session["CompanyId"].ToString() : "";
                var resultStream = _repo.IncoiceReportPreview(param);
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    Response.Headers.Add("Content-Disposition", "inline; filename=SaleDeliveryInvoice_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf");
        ...
        public ActionResult SummaryReport(string fromDate, string toDate, string branchId, string type, string isPost)
        {
            ...
            try
            {
                ...
                string filename = "Sale Delivery Summary Report " + "-" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;

                ExcelPackage excel = new ExcelPackage();
                var workSheet = excel.Workbook.Worksheets.Add("Sale Delivery Summary Report");
        ...
        public void ExcelSheetFormat(DataTable dt, ExcelWorksheet workSheet, string fromDate, string toDate, string BranchName, string BranchAddress, string CompanyName)
        {
            ...
            string reportHeader = "Sale Delivery Summary Report";
        ...
        public ActionResult InvoiceWiseSaleDeliveryIndex()
        {
            SaleDeliveryVM vm = new SaleDeliveryVM();
            var currentBranchId = Session["CurrentBranch"] != null ? Session["CurrentBranch"].ToString() : "0";
            vm.BranchId = Convert.ToInt32(currentBranchId);
        ...
        [HttpPost]
        public JsonResult GetDeliveryNoWiseGridData(GridOptions options, string branchId, string fromDate, string toDate)
        {
            ResultVM result = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new SaleDeliveryRepo();
            try
            {
                ...
                options.vm.FromDate = fromDate;
                options.vm.ToDate = toDate;
                result = _repo.GetDeliveryNoWiseGridData(options);
        ...
        [HttpPost]
        public JsonResult GetSaleDueListByCustomerId(GridOptions options, string customerId)
        {
            ResultVM result = new ResultVM { Status = "Fail", Message = "Error", ExMessage = null, Id = "0", DataVM = null };
            _repo = new SaleDeliveryRepo();
            ...
            try
            {
                options.vm.Id = customerId == "0" ? "" : customerId;               
                result = _repo.GetSaleDueListByCustomerId(options);
