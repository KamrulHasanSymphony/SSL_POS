...
namespace ShampanPOSUI.Areas.DMS.Controllers
{
    public class PurchaseOrderController : Controller
    {
        ...
        {
            PurchaseOrderDetail vm = new PurchaseOrderDetail();
        ...
        [HttpPost]
        public ActionResult Delete(PurchaseOrderVM vm)
        {
            ResultModel<PurchaseOrderVM> result = new ResultModel<PurchaseOrderVM>();
        ...
        public JsonResult GetDetailsGridData(GridOptions options, string branchId, string isPost, string fromDate, string toDate)
        {
            ...
            try
            {
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var gridData = JsonConvert.DeserializeObject<GridEntity<PurchaseOrderDetail>>(result.DataVM.ToString());
        ...
        public async Task<ActionResult> ReportPreview(CommonVM param)
        {
            try
            {
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    Response.Headers.Add("Content-Disposition", "inline; filename=PurchaseOrder_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf");
            ...
            catch (Exception e)
            {
                ...
                return RedirectToAction("Index", "PurchaseOrder", new { area = "DMS", message = TempData["Message"] });
            }
        }
        ...
        public ActionResult Report()
        {
            try
            {
                ...
                var branchId = Session["CurrentBranch"].ToString();
                purchaseOrderVM.BranchId = Convert.ToInt32(branchId);
                purchaseOrderVM.Branchs = purchaseOrderVM.BranchId;
                DateTime currentDate = DateTime.Now;
                DateTime firstDayOfMonth = new DateTime(currentDate.Year, currentDate.Month, 1);
                DateTime lastDayOfMonth = firstDayOfMonth.AddMonths(1).AddDays(-1);
                purchaseOrderVM.FromDate = firstDayOfMonth.ToString("yyyy/MM/dd");
                purchaseOrderVM.ToDate = lastDayOfMonth.ToString("yyyy/MM/dd");
        ...
        public ActionResult SummaryReport(string fromDate, string toDate, string branchId,string type,string isPost)
        {
            ...
            try
            {
               ...
               var result = _repo.SummaryReport(vm);
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var data = JsonConvert.DeserializeObject<List<PurchaseOrderReportVM>>(result.DataVM.ToString());
                ...
                string filename = "Purchase Order Summary Report " + "-" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;

                ExcelPackage excel = new ExcelPackage();
                var workSheet = excel.Workbook.Worksheets.Add("Purchase Order Summary Report");
        ...
        public void ExcelSheetFormat(DataTable dt, ExcelWorksheet workSheet, string fromDate, string toDate, string BranchName, string BranchAddress, string CompanyName)
        {
            ...
            string reportHeader = "Purchase Order Summary Report";
