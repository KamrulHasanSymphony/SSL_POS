...
namespace ShampanPOSUI.Areas.DMS.Controllers
{
    ...
    public class PurchaseReturnController : Controller
    {
        ...
        PurchaseReturnRepo _repo = new PurchaseReturnRepo();
        CommonRepo _commonRepo = new CommonRepo();

        // GET: DMS/PurchaseReturn
        ...
        {
            PurchaseReturnDetail vm = new PurchaseReturnDetail();
        ...
        public ActionResult Create()
        {
            PurchaseReturnVM vm = new PurchaseReturnVM();
            vm.Operation = "add";
			vm.TransactionType = "PurchaseReturn";
			var currentBranchId = Session["CurrentBranch"] != null ? Session["CurrentBranch"].ToString() : "0";
            vm.BranchId = Convert.ToInt32(currentBranchId);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    if (model.Operation.ToLower() == "add")
                    {
                        ...
                        {
                            model = JsonConvert.DeserializeObject<PurchaseReturnVM>(resultVM.DataVM.ToString());
                            model.Operation = "add";
        ...
        public ActionResult NextPrevious(int id, string status)
        {
            ...
            try
            {
                ...
                vm.TableName = "PurchaseReturns";

                ResultVM result = _commonRepo.NextPrevious(vm);

                if (result.Id != "0")
                {
                    string url = Url.Action("Edit", "PurchaseReturn", new { area = "DMS", id = result.Id });
                    return Redirect(url);
                }
                else
                {
                    string url = Url.Action("Edit", "PurchaseReturn", new { area = "DMS", id = id });
                    return Redirect(url);
                }
            }
            catch (Exception e)
        ...
        [HttpGet]
        public JsonResult GetPurchaseReturnDetailDataById(GridOptions options, int masterId)
        {
            ...
            try
            {
                result = _repo.GetPurchaseReturnDetailDataById(options, masterId);

                if (result.Status == "Success" && result.DataVM != null)
                {
                    var gridData = JsonConvert.DeserializeObject<GridEntity<PurchaseReturnDetailVM>>(result.DataVM.ToString());
        ...
        [HttpPost]
        public ActionResult Delete(PurchaseReturnVM vm)
        {
            ResultModel<PurchaseReturnVM> result = new ResultModel<PurchaseReturnVM>();
        ...
        public JsonResult GetGridData(GridOptions options, string branchId, string isPost, string fromDate, string toDate)
        {
            ...
            try
            {
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var gridData = JsonConvert.DeserializeObject<GridEntity<PurchaseReturnVM>>(result.DataVM.ToString());
        ...
        public JsonResult GetDetailsGridData(GridOptions options, string branchId, string isPost, string fromDate, string toDate)
        {
            ...
            try
            {
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var gridData = JsonConvert.DeserializeObject<GridEntity<PurchaseReturnDetail>>(result.DataVM.ToString());
        ...
        [HttpGet]
        public ActionResult FromPurchase()
        {
            PurchaseReturnVM vm = new PurchaseReturnVM();
            var currentBranchId = Session["CurrentBranch"] != null ? Session["CurrentBranch"].ToString() : "0";
            vm.BranchId = Convert.ToInt32(currentBranchId);
        ...
        public ActionResult GetFromPurchase(CommonVM vm)
        {
            try
            {
                ...
                PurchaseReturnVM purchaseReturn = new PurchaseReturnVM();
                ResultVM result = _repoo.PurchaseList(vm);

                if (result.Status == "Success" && result.DataVM != null)
                {
                    purchaseReturn = JsonConvert.DeserializeObject<List<PurchaseReturnVM>>(result.DataVM.ToString()).FirstOrDefault();
                }
                else
                {
                    TempData["message"] = result.Message;
                    return RedirectToAction("FromPurchase", "PurchaseReturn", new { area = "DMS" });
                }
        ...
        public async Task<ActionResult> ReportPreview(CommonVM param)
        {
            try
            {
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    Response.Headers.Add("Content-Disposition", "inline; filename=PurchaseReturn_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf");
            ...
            catch (Exception e)
            {
                ...
                return RedirectToAction("Index", "PurchaseReturn", new { area = "DMS", message = TempData["Message"] });
            }
        }
        ...
        public ActionResult SummaryReport(string fromDate, string toDate, string branchId, string type, string isPost)
        {
            ...
            try
            {
                ...
                if (result.Status == "Success" && result.DataVM != null)
                {
                    var data = JsonConvert.DeserializeObject<List<PurchaseReturnReportVM>>(result.DataVM.ToString());
                ...
                string filename = "Purchase Return Summary Report " + "-" + DateTime.Now.ToString("yyyyMMddHHmmss");
                ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;

                ExcelPackage excel = new ExcelPackage();
                var workSheet = excel.Workbook.Worksheets.Add("Purchase Return Summary Report");
        ...
        public void ExcelSheetFormat(DataTable dt, ExcelWorksheet workSheet, string fromDate, string toDate, string BranchName, string BranchAddress, string CompanyName)
        {
            ...
            string reportHeader = "Purchase Return Summary Report";
