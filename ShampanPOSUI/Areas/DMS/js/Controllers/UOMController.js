var UOMController = function (CommonAjaxService) {
    ...
    var init = function () {
        ...
        $('#btnDeleteUOMConversation').on('click', function () {

            Confirmation("Are you sure? Do You Want to Delete Data?",
                function (result) {
                    if (result) {
                        SelectDataUOMConversation();
                    }
                });
        });
        ...
        $('#btnPrevious').click('click', function () {
            var getId = $('#Id').val();
            if (parseInt(getId) > 0) {
                window.location.href = "/DMS/UOM/NextPrevious?id=" + getId + "&status=Previous";
            }
        });
        $('#btnNext').click('click', function () {
            var getId = $('#Id').val();
            if (parseInt(getId) > 0) {
                window.location.href = "/DMS/UOM/NextPrevious?id=" + getId + "&status=Next";
            }
        });
    ...
    function SelectDataUOMConversation() {

        var IDs = [];

        var selectedRows = $("#GridDataListofUOMConversation").data("kendoGrid").select();
        ...
        selectedRows.each(function () {
            var dataItem = $("#GridDataListofUOMConversation").data("kendoGrid").dataItem(this);
            IDs.push(dataItem.Id);
        });
            ...
            columns: [
                ...
                },
                {
                    ...
                    template: function (dataItem) {
                        ...
                        return `
                                 <a href="/DMS/UOM/Edit/${dataItem.Id}" class="btn btn-primary btn-sm mr-2 edit">
    ...
    var GetGridDataListofUOMConversation = function () {
        getFromId = $("#Id").val()
        var gridDataSource = new kendo.data.DataSource({
            type: "json",
            serverPaging: true,
        ...
        $("#GridDataListofUOMConversation").kendoGrid({
            dataSource: gridDataSource,
            pageable: {
                refresh: true,
                serverPaging: true,
            ...
            excel: {
                fileName: "UomConversations.xlsx",
                filterable: true
            },
            pdf: {
            ...
            columns: [
                ...
                { field: "FromName", title: "From",  sortable: true },
                { field: "ToName", title: "To Name", sortable: true },
                { field: "ConversationFactor", title: "Conversation Factor", sortable: true},
                {
                    field: "Status", title: "Status", sortable: true,
                    filterable: {
                        ui: function (element) {
