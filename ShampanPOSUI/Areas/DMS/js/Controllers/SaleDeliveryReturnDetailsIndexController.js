var SaleDeliveryReturnDetailsIndexController = function (CommonService, CommonAjaxService) {
    ...
    var GetGridDetailsDataList = function () {
        ...
        var gridDataSource = new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: "/SaleDeliveryReturn/GetDetailsGridData",
                    type: "POST",
                    dataType: "json",
                    cache: false,
        ...
        $("#GridDetailsDataList").kendoGrid({
            ...
            search: ["Code", "BranchName", "CustomerName", "SalePersonName", "RouteName", "ProductName", "UOMName", "DeliveryAddress", "InvoiceDateTime", "DeliveryDate", "GrandTotalAmount", "GrandTotalSDAmount", "GrandTotalVATAmount", "Comments", "TransactionType", "IsCompleted", "Quantity", "UnitRate", "SubTotal", "SD", "SDAmount", "VATRate", "VATAmount", "LineTotal", "UOMName", "CurrencyName"],
            excel: {
                fileName: `SaleDelivery_Return_Details_${new Date().toISOString().split('T')[0]}.xlsx`,
                filterable: true
            },
            pdf: {
                fileName: `SaleDelivery_Return_Details_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0]}.${new Date().getMilliseconds()}.pdf`,
                allPages: true,
                avoidLink: true,
                filterable: true
            },
            pdfExport: function (e) {
                ...
                var fileName = `SaleDelivery_Return_Details_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0]}.${new Date().getMilliseconds()}.pdf`;
