var OverHeadController = function (CommonService, CommonAjaxService) {
    ...
    var init = function () {
        ...
        $('#btnPrevious').click('click', function () {
            var getId = $('#Id').val();
            if (parseInt(getId) > 0) {
                window.location.href = "/DMS/OverHead/NextPrevious?id=" + getId + "&status=Previous";
            }
        });
        $('#btnNext').click('click', function () {
            var getId = $('#Id').val();
            if (parseInt(getId) > 0) {
                window.location.href = "/DMS/OverHead/NextPrevious?id=" + getId + "&status=Next";
            }
        });
    ...
    var GetGridDataList = function () {
        ...
        $("#GridDataList").kendoGrid({
            ...
            pdf: {
                fileName: `OverHeads_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0]}.${new Date().getMilliseconds()}.pdf`,
                allPages: true,
                avoidLink: true,
                filterable: true
            },
            pdfExport: function (e) {
                ...
                var fileName = `OverHeads_${new Date().toISOString().split('T')[0]}_${new Date().toTimeString().split(' ')[0]}.${new Date().getMilliseconds()}.pdf`;
            ...
            columns: [
                ...
                },
                {
                    ...
                    template: function (dataItem) {
                        ...
                        return `
            <a href="/DMS/OverHead/Edit/${dataItem.Id}" class="btn btn-primary btn-sm mr-2 edit" title="Edit Credit Limit">
                ...
                { field: "OverHead", title: "Over Head", sortable: true, width: 200 },
    ...
    function save() {
        ...
        var url = "/DMS/OverHead/CreateEdit";
...
function ReportPreview(id) {
    ...
    form.method = 'post';
    form.action = '/DMS/OverHead/ReportPreview';
    form.target = '_blank';
...
function DevitCredit(branchId, FromDate, ToDate) {
    ...
    const form = document.createElement('form');
    form.method = 'post';
    form.action = '/DMS/OverHead/DevitCredit';
    form.target = '_blank';
    const inputVal = document.createElement('input');
    inputVal.type = 'hidden';
    inputVal.name = 'Id';
